  ...
  return (
    <div style={{
      ...
      <div className="glass-lg" style={{ maxWidth: 600, width: '100%', maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
        ...
        {/* Header */}
        <div style={{ padding: '20pstifyContent: 'space-between', alignItems: 'center' }}>
          <div>
        ...
        <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border)', display: 'flex', gap: '24px' }}>
          ...
          <div>
 fontSize: '11px', color: 'var(--text-dim)', textTransform: 'uppercase' }}>Visits</div>
...
lex', justifyContent: 'space-between', alignItems: 'center',
      ...
      {open && (
        <div style={{ padding: '10px 14px', borderTop: '1px solid var(--border)', background: 'transparent' }}>
          {order.products.map((p, i) => (
            <div key={i} style={{ display: 'flex', justif-between', fontSize: '12px', padding: '3px 0', color: 'var(--text-dim)' }}>
  ...
  return (
    <div style={{ maxWidth:padding: '32px 24px' }}>
      ...
      {loading ? (
        ...
        <>
          {/* Stat cards */}
          <div style={{ dispfr))', gap: '16px', marginBottom: '28px' }}>
      ...
      {selectedEntry && (
        ...
        />
                    ...
                    <td style={{ padding: '10px 12px', color: 'var(--text-dim)' }}>{o.orderDate}</td>
                          ...
                          <td style={{ padding: '10px 12px', fontWeight: 600 }}>₹{o.to{ padding: '8px 12px', textAlign: 'left', color: 'var(--text-dim)', fontWeight: 500 }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
      ...
      th === 0 ? (
                    ...
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--border)' }}>
                        {['Order', 'Date', 'Products', 'Method', 'Total'].map(h => (
                          <th key={h} style={  <td style={{ padding: '10px 12px', color: 'var(--text-dim)' }}>{entry.visitCount}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="glass" style={{ padding: '20px', gridColumn: '1 / -1' }}>
                    ...
                    <tbody>
                      {ranked.map(entry => (
                        <tr key={entry.customer.customerId}
                            ...
                            onMouseEnter={e => e.currentTarget.style.backgroun
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    </t       <Users size={16} color="var(--gold)" />
              ...
              {ranked.length === 0 ? (
                ...
                <div style={{ overflowX: 'auto' }}>0%', borderRadius: '3px', width: `${pct}%`,
                        background: METHOD_COLORS[m] || '#888', transition: 'width 0.6s ease',
                      }} />
                    </div>
                  </div>
                );
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '5px' }}>
                    ...
                    <div style={{ height: '6px', background: 'var(--bg2)', borderRadius: '3px', overflow: 'hidden' }}>
                      <div style={{
                        height: '1016px', fontSize: '14px' }}>Payment Methods</div>
            ...
            {/* Payment methods */}
            <div className="glass" style={{ padding: '20px' }}>
              <div style={{ fontWeight: 600, marginBottom: ', qty]) => (
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            <div className="glass" style={{ padding: '20px' }}>
              <div style={{ fontWeight: 600, marginBottom: '16px', fontSize: '14px' }}>Top Products by Quantity</div>
              {topProducts.length === 0 ? <p style={{ color: 'var(--text-dim)', fontSize: '13px' }}>No data yet</p>
                : topProducts.map(([name, qty]) => (
                  <div key={name} style={{ marginBottom: '12px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '5px' }}>
                      <span>{name}</span><span className="gold-text" style={{ fontWeight: 600 }}>{qty}</span>
                    </div>
                    <div style={{ height: '6px', background: 'var(--bg2)', borderRadius: '3px', overflow: 'hidden' }}>
            ...
            <div className="glass" style={{ padding: '20px' }}>
              ...
              {Object.keys(methodCounts).length === 0 ? <p style={{ color: 'var(--text-dim)', fontSize: '13px' }}>No data yet</p>
                : Object.entries(methodCounts).map(([m, count]) => {
                  const total = Object.values(methodCounts).reduce((a,b) => a+b, 0);
                  const pct = Math.round((count/total)*100);
                  return (
            ...
            <div className="glass" style={{ padding: '20px', gridColumn: '1 / -1' }}>
              <div style={{ fontWeight: 600, marginBottom: '16px', fontSize: '14px' }}>Recent Orders</div>
              {orders.length === 0 ? <p style={{ color: 'var(--text-dim)', fontSize: '13px' }}>No orders yet</p> : (
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--border)' }}>
